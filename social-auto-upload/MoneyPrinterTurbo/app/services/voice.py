...
def mktimestamp(time_100ns: int) -> str:
    from datetime import timedelta
    td = timedelta(microseconds=time_100ns / 10)
...
class MptSubMaker:
    ...
    def create_sub(self, duration, text):
        self.subs.append(text)
        self.offset.append(duration)
...
def azure_tts_v1(
    ...
    for i in range(3):
        try:
            ...
            async def _do() -> MptSubMaker:
                communicate = edge_tts.Communicate(text, voice_name, rate=rate_str)
                sub_maker = MptSubMaker()
...
def get_audio_duration( target: Union[str, Any]) -> float:
    """
    获取音频时长
    如果是SubMaker对象，则从SubMaker中获取时长
    如果是MP3文件，则从MP3文件中获取时长
    """
    if isinstance(target, (submaker.SubMaker, MptSubMaker)):
