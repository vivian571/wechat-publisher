...
class WeChatPublisher:
    ...
    def _get_remote_image_as_temp_file(self, title):
        ...
        if pexels_api_key:
            try:
                ...
                logging.error(f"使用 Pexels API 搜索图片时出错 (可能触发了 SystemExit): {e}", extra={'account_name': self.account_name})
        ...
        if not images:
            ...
            if new_image_url:
                ...
                if re.search(r'^#\s*.*', md_content, re.MULTILINE):
                     md_content = re.sub(r'(^#\s*.*(?:\n|\r\n))', r'\g<1>' + image_md, md_content, count=1)
        ...
        try:
            ...
            if "access_token" in data:
                ...
                if err_code == 40164:
                    # 提取 IP 地址
                    import re
                    ip_match = re.search(r'invalid ip ([\d\.]+)', err_msg)
                    ip = ip_match.group(1) if ip_match else "未知 IP"
                    err_msg = f"IP {ip} 不在白名单中。请登录微信公众平台 -> [设置与开发] -> [基本配置] -> [IP白名单] 中添加该 IP。"
                
                logging.error(f"获取access_token失败: {err_msg}", extra={'account_name': self.account_name})
                return False, err_msg
        except requests.exceptions.RequestException as e:
            err_msg = f"请求access_token时出错: {e}"
            logging.error(err_msg, extra={'account_name': self.account_name})
            return False, err_msg

    def ensure_token_valid(self):
        if not self.access_token or time.time() >= self.token_expires_at:
            success, err_msg = self.get_access_token()
            if not success:
                raise Exception(f"无法获取或刷新 access_token: {err_msg}")
    ...
    def _upload_media(self, image_path, url, return_key, media_type=None):
        ...
        try:
            with open(image_path, 'rb') as f:
                ...
                # 增加超时时间到 90s，支持大文件上传
                response = self.session.post(url, files=files, params=params, timeout=90)
    ...
    def create_draft(self, title, content, thumb_media_id, author, digest):
        ...
        try:
            # 增加超时到 60s
            response = self.session.post(url, data=json.dumps(data, ensure_ascii=False).encode('utf-8'), timeout=60)
            response.raise_for_status()
